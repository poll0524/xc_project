...
public class ShareServiceIpml implements IShareService {
     ...
     * @return
     */
    public Object insertShareUser(Map<String,Object> share) throws JSONException {
        ...
        if (start == 0){
            ...
            if (shareUser != null){
                ...
                return shareUserMapper.selectShareUserMeds(openId,readOpenId,articleId,sign).getShare_id();
            }
            ...
            if (shareUser != null){
                ...
                return shareUserMapper.selectShareUserVid(openId,readOpenId,videoId,sign).getShare_id();
            }
    ...
    public Map<String,Object> readUser(Map<String,Object> users) throws ParseException {
        ...
        if (start == 0){
            for (ShareUser shareUser : shareUsers) {
                ...
                String readTime = shareUser.getRead_time();
                if (readTime == null){
                    data.put("readTime","老版本数据,阅读时间未记录");
                }else {
                    Integer readTimeInt = Integer.parseInt(readTime);
                    if (readTimeInt <= 60){
                        data.put("readTime","00:00:"+readTime);
                    }else {
                        int h = readTimeInt/3600;
                        int m = (readTimeInt%3600)/60;
                        int s = (readTimeInt%3600)%60;
                        data.put("readTime",h+":"+m+":"+s);
                    }
                }
                datas.add(data);
            }
...
//                courseInfo.put("price",course.getPrice());
     ...
     * 阅读时长
     */
    public String readTime(Map<String,Object> data){
        Integer shareId = (Integer) data.get("shareId");//素材的id//        String endTime = (String) data.get("endTime");
        Integer endTime = (Integer) data.get("endTime");//秒

        ShareUser shareUser = shareUserMapper.selectShareId(shareId);
        String shareEndTime = shareUser.getRead_time();
        if (shareEndTime == null){
            shareUserMapper.updataEndTime(shareId,endTime.toString());
            return "时间记录成功";
        }
        Integer endTimes = endTime + Integer.parseInt(shareEndTime);
        shareUserMapper.updataEndTime(shareId,endTimes.toString());
