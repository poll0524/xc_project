...
import com.example.xcschoolserver.mapper.accessTokenMapper;
...
public class AccessTokenUtil {
    ...
    public String getAccessToken() throws JSONException {
        ...
        if( accessTokenSql.getToken_info() == null || accessTokenSql.getToken_time() == null){
            ...
            //本地
            String result = HttpUtil.get(WeChatUtil.GET_ACCESSTOKEN_URL.replace("APPID","wx4cb0f21739ce9406").replace("APPSECRET","8c19ef4a0f13b6e57ea34fb1add2c962"));
            //线上
//            String result = HttpUtil.get(WeChatUtil.GET_ACCESSTOKEN_URL.replace("APPID","wx6a0dcf623263d8c0").replace("APPSECRET","85b40c1e4c90e73aa72ac6e8db84ce78"));
    ...
    public Map<String,Object> getWebAccessToken(String code) throws JSONException {
        ...
        if (RefreshToken.getToken_time() == null || RefreshToken.getToken_info() == null){//当RefreshToken或时间为空时
            ...
            if( webaccesstoken.getToken_info() == null || webaccesstoken.getToken_time() == null){
                ...
                //本地
                String result = HttpUtil.get(WeChatUtil.GET_REFRESH_TOKEN.replace("APPID","wx4cb0f21739ce9406").replace("REFRESH_TOKEN",accessTokenMapper.selectNameAccessToken(WeChatUtil.REFRESHTOKEN).getToken_info()));
                //线上
//                String result = HttpUtil.get(WeChatUtil.GET_REFRESH_TOKEN.replace("APPID","wx6a0dcf623263d8c0").replace("REFRESH_TOKEN",accessTokenMapper.selectNameAccessToken(WeChatUtil.REFRESHTOKEN).getToken_info()));
...
//                String result = HttpUtil.get(WeChatUtil.GET_REFRESH_TOKEN.replace("APPID","wx6a0dcf623263d8c0").replace("REFRESH_TOKEN",accessTokenMapper.selectNameAccessToken(WeChatUtil.REFRESHTOKEN).getToken_info()));
    ...
    public Map<String,Object> getWebaccessTokens(String code) throws JSONException {
        //发起get请求获取webaccess_token
        //本地
        String result = HttpUtil.get(WeChatUtil.GET_WEBACCESSTOKEN_URL.replace("APPID","wx4cb0f21739ce9406").replace("SECRET","8c19ef4a0f13b6e57ea34fb1add2c962").replace("CODE",code));
        //线上
//        String result = HttpUtil.get(WeChatUtil.GET_WEBACCESSTOKEN_URL.replace("APPID","wx6a0dcf623263d8c0").replace("SECRET","85b40c1e4c90e73aa72ac6e8db84ce78").replace("CODE",code));
...
//                "}";
        ...
        String string = "{\n" +
                ...
                "       \"type\":\"view\",\n" +
                "       \"name\":\"首页\",\n" +
                "       \"url\":\"http://classroom.xiaocisw.site/home?index=0\",\n" +
                "       \"sub_button\":[ ]\n" +
                "   },\n" +
                "    {\n" +
                "       \"type\":\"view\",\n" +
                "       \"name\":\"个人中心\",\n" +
                "       \"url\":\"http://classroom.xiaocisw.site/settings?index=3\",\n" +
                "       \"sub_button\":[ ]\n" +
                "   },\n" +
                "    {\n" +
                "       \"type\":\"view\",\n" +
                "       \"name\":\"帮助中心\",\n" +
                "       \"url\":\"http://classroom.xiaocisw.site/helpCenter\",\n" +
                "       \"sub_button\":[ ]\n" +
                "   }\n" +
                "   ]\n" +
                "}";
