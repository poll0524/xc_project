...
import com.example.xcschoolserver.mapper.brandNavMapper;
...
public class SpiderUtil {
    private static brandNavMapper brandNavMapper;
     ...
     * @return 文章内容
     */
    public static Map getActicle(String url) {
     ...
     * 公众号聊天框采集文章
     */
    public static Map getActicles(String url,String openId) {

        // 请求与响应
        String resp = HttpTool.get(url, getWxHeaderMap());

        // 解析
        Map acticleResp = getWxActicleContents(resp,openId);
     ...
     * 公众号聊天框采集文章
     */
    public static Map getWxActicleContents(String resp, String openId) {
        try {
            Document document = Jsoup.parse(resp);
            // 文章出处（作者）
            String referName = document.getElementsByClass("profile_nickname").get(0).text();
